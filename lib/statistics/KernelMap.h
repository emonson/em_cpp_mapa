...
class KernelMap{
  ...
  public:
   ...
   //lout - leave one out 
   KernelMap(DenseMatrix<TPrecision> Ydata, DenseMatrix<TPrecision> Xinit, 
   ...
   KernelMap(DenseMatrix<TPrecision> Ydata, DenseMatrix<TPrecision> Xinit, 
       int nn, double sigmaX, bool lout): Y(Ydata), X(Xinit), knn(nn){
   ...
   void f( Vector<TPrecision> &y, Vector<TPrecision> &out, int lo = -1 ){
     ...
     TPrecision sum = 0;
     int index = 0;kernelY = new GaussianKernel<TPrecision>[Y.N()];
     for(int i=0; i<Y.N(); i++){
       if(index == lo){
         k(index) = 0;
       }
       else{
         k(index) = kernelY.f(y, Y, i); 
         sum += k(index);
       }
       ++index;
     }
  ...
  void g( int index, Vector<TPrecision> &out ){
   ...
   void gradX( int index, DenseVector<TPrecision> &gx, TPrecision kCutY, 
     ...
     for(int is=0; is < nY; is++){
      ...
      //Gradient matrix of (g \circ f)(f(y)) for each component of \hat{x}_index 
...
private:
  ...
  void computeKernelX(TPrecision alpha){
    ...
    sigma/=fY.N();
    kernelX = GaussianKernel<TPrecision>(alpha*sigma);
  ...
  void computeKernelY(){
    ...
    sigma /= Y.N();
    kernelY = GaussianKernel<TPrecision>(3*sigma);
