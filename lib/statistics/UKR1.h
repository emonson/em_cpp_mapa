...
class UKR{
  ...
  void g(unsigned int index, Vector<TPrecision> &out){
     ...
     for(int i=0; i<Z.N(); i++){
      TPrecision w = kernelX.f(Z, index, Z, i);
   ...
   void jacobian(Vector<TPrecision> &x, Matrix<TPrecision> &J){
     Linalg<TPrecision>::Zero(J); 
     DenseVector<TPrecision> gx(Z.M());
     for(unsigned int i=0; i<Z.N(); i++){
       TPrecision w = kernelX.gradf(x, Z, i, gx);
       Linalg<TPrecision>::AddOuterProduct(J, gx, Y, i, J);
     }
     gx.deallocate();
   };
   
   void jacobian(int index, Matrix<TPrecision> &J){
     Linalg<TPrecision>::Zero(J); 
     DenseVector<TPrecision> gx(Z.M());
     for(unsigned int i=0; i<Z.N(); i++){
       TPrecision w = kernelX.gradf(Z, index, Z, i, gx);
       Linalg<TPrecision>::AddOuterProduct(J, gx, Y, i, J);
     }
     gx.deallocate();
   };
