#ifndef AAKERNELMAP_H
#define AAKERNELMAP_H
...
class AAKernelMap{
  ...
  private:
    ...
    GaussianKernel<TPrecision> kernelW;
    AnistropicGaussianKernel<TPrecision> *kernelY;
    AnistropicGaussianKernel<TPrecision> kernelYtmp;
  ...
  public:
   ...
   //lout - leave one out 
   AAKernelMap(DenseMatrix<TPrecision> Ydata, DenseMatrix<TPrecision> Xinit, 
       TPrecision alpha, int nn, bool lout): Y(Ydata), X(Xinit), knn(nn){
     leaveout = lout;
     kernelY = new AnistropicGaussianKernel<TPrecision>[Y.N()];
   ...
   void f( DenseVector<TPrecision> &y, DenseVector<TPrecision> &out, int lo = -1 ){
     ...
     //DenseVector<TPrecision> mean = SampleStatistics<TPrecision>::Mean(Y, w);
     DenseMatrix<TPrecision> cov = SampleStatistics<TPrecision>::Covariance(Y, y, w);
     kernelYtmp.setKernelParam(cov); 
...
private:
  ...
  void computeKY(){
    ...
    TPrecision sigmaY = 0;
    for(int i=0; i < Y.N(); i++){
      sigmaY += knnDists(knn, i);
    }
    sigmaY /= Y.N();
    kernelW.setKernelParam(sigmaY);

    DenseVector<TPrecision> w(Y.N());
    for(int i=0; i<N; i++){
      for(int j=0; j < N; j++){
        w(j) = kernelW.f(Y, i, Y, j);
      }
      w(i) = 0;
      ...
      DenseMatrix<TPrecision> cov = SampleStatistics<TPrecision>::Covariance(Y, mean, w);
      kernelY[i].setKernelParam(cov); 
