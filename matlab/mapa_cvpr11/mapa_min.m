function [labels, planeDims, planeCenters, planeBases] = mapa_min(X,opts)
...
if isfield(opts, 'K'),
    ...
    [planeDims, labels, err, planeCenters, planeBases] = ...
...
elseif isfield(opts, 'Kmax'),
    ...
    disp(['L2Error K=' num2str(K) ': ' num2str(L2Errors)]);
    while K<opts.Kmax && L2Errors > 1.05*eps
        K = K+1;
        [planeDims, labels, L2Errors, planeCenters, planeBases] = ...
            spectral_analysis(X, U(:,1:K), allPtsInOptRegions, invColMap, localDims, opts.nOutliers);
        disp(['L2Error ' num2str(K) ': ' num2str(L2Errors)]);
    end
